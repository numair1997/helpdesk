"use client";
import logo from "@/public/images/helpdesk-logo.svg";
          ...
          <div className="text-s1 flex items-center justify-between py-6">
            <div className="flex items-center justify-start gap-3 pb-1">
              ...
              <Link href="/">
                <Image
                  src={logo}
                  alt="Servibe logo"
                  className="max-w-[170px] max-sm:max-w-[150px] text-white"
                />
              </Link>
            </div>

            <nav className="max-lg:hidden">
              <ul className="flex items-center justify-center font-medium xxl:gap-16">
                <li>Home</li>
                <li>Services</li>
                <li>About Us</li>
                <li>Pricing</li>
                  ...
                  <li key={menu.id}>
                    {menu.isSubmenu && (
                      <div className="group relative cursor-pointer">
                        <div
                          className={`subMenuTitle relative flex items-center justify-center gap-1 px-2 py-3 hover:text-b500 ${isMenuActive(menu) ? "text-b500" : ""
                            }`}
                        >
                          {menu.name}
                        ...
                        <ul className="group-hover:eventunset pointer-events-none invisible absolute rtl:right-0 ltl:left-0 top-12 flex w-[220px] translate-y-8 scale-75 flex-col items-start justify-start gap-3 rounded-lg bg-b300 py-6 font-medium text-white/90 opacity-0 duration-500 group-hover:visible group-hover:z-50 group-hover:translate-y-0 group-hover:scale-100 group-hover:opacity-100">
                          {menu.submenu?.map(({ id, name, link }) => (
                            <li key={id}>
                              <Link
                                href={link}
                                className={`subMenuItem px-6 duration-500 hover:ml-2 hover:text-y200 ${path === link && "text-y200"
                                  }`}
                              >
                                {name}
                              </Link>
            ...
            <div className="flex items-center justify-between gap-6">
              ...
              <div className="flex items-center justify-between gap-3 font-semibold">
                <Link
                  href="/"
                  className="bg-white text-black p-2 pl-4 pr-4  rounded-3xl"
